    ...
    <nav className="bg-[#162029] p-5 fixed z-50 w-full">
      <div className="container mx-auto flex flex-wrap items-center justify-between">
        ...
        <div className="flex items-center flex-shrink-0 text-white w-[300px]">
          <span className="font-semibold text-xl tracking-tight"><Link to='/'>Accommodate</Link></span>
        </div>
        ...
        <div className={`w-full flex-grow lg:flex lg:items-center lg:w-auto ${isOpen ? 'block' : 'hidden'}`}>
          <div className="text-sm lg:flex-grow text-center lg:text-left">
           <Link to='/' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-[#0BA75A] mx-4">
             Home </Link> 
           <Link to='/hostel' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-[#0BA75A] mx-4">
            Hostel  </Link> 
           <Link to='/payment' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-[#0BA75A] mx-4">
           Payment   </Link>
          </div>
